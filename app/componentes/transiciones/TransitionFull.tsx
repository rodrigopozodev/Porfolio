 ...
 * Por defecto la animación NO se lanza al iniciar (skip=true, autoExit=false).
 * Esta transición se usa para el botón de temas (ThemeToggle).
 */
...
const Transition: React.FC<TransitionProps> = ({
  ...
  // No mostrar intro al inicio y no salir automáticamente
  skip = true,
  autoExit = false,
  trigger,
  onFinished,
  ...
  // Usada por el botón de temas (ThemeToggle) mediante el evento "themeToggleTransition".
  useEffect(() => {
  ...
  return (
    <div ref={ref} className="relative w-full h-full min-h-full">
      {/* Contenido de la página (siempre visible, la cortinilla se dibuja encima cuando showIntro=true) */}
      <div className="relative z-0 w-full h-full">{children}</div>

      {/* Cortinilla de transición: sólo aparece cuando se solicita explícitamente */}
      {showIntro && (
        <div
